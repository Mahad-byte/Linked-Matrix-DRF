...
class DocumentAPITest(APITestCase):
    ...
    def test_create_document_via_api(self):
        proj_resp = self.client.post("/api/projects/", {"title": "Project A", "description": "desc"}, format="json")
        ...
        data = {"name": "Doc1", "description": "desc", "file": file, "version": 1, "project": project_id}
        resp = self.client.post("/api/documents/", data, format='multipart')
        self.assertEqual(resp.status_code, status.HTTP_201_CREATED)
        self.assertEqual(Document.objects.count(), 1)

    def test_missing_file_returns_400(self):
        proj_resp = self.client.post("/api/projects/", {"title": "Project B", "description": "desc"}, format="json")
        project_id = proj_resp.data["id"]
        data = {"name": "Doc2", "description": "desc", "version": 1, "project": project_id}
        resp = self.client.post("/api/documents/", data, format='multipart')
